// Mirror of drawparty-backend/src/types/socket.ts — keep in sync.
...
export type ClientToServerEvents = {
  ...
  "room:update_settings": (payload: Partial<LobbySettings>) => void;
  ...
  "skribbl:submit_drawing":    (payload: { roomId: string; imageBase64: string }) => void;
...
export type ServerToClientEvents = {
  ...
  "gartic:rewind_data":   (payload: { chains: GarticChainDTO[] }) => void;
  ...
  "friend:request_received": (payload: { requestId: string; from: { clerkId: string; username: string; avatarUrl: string | null } }) => void;
  "room:invite_received":    (payload: { roomId: string; roomCode: string; fromId: string; fromUsername: string; gameMode: "GARTIC_PHONE" | "SKRIBBL" }) => void;
  "friend:removed":          (payload: { removedBy: string }) => void;
  "friend:request_accepted": (payload: { friend: { id: string; clerkId: string; username: string; avatarUrl: string | null } }) => void;
};
